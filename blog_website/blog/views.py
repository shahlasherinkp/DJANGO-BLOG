...
from .forms import TextForm, AddlogForm
...
def search_blogs(request):
    ...
    tags = Tag.objects.order_by('-created_dtae')
    if search_key:
        blogs = Blog.objects.filter(
            Q(title_icontains=search_key) |
            Q(category__title__icontains=search_key) |
            Q(user__username__icontains=search_key) |
            Q(tags_title_icontains=search_key)
        ).distinct()
...
def add_blog(request):
    ...
    if request.method == "POST":
        form = AddlogForm(request.POST, request.FILES)
        if form.is_valid():
                ...
                if tag_input.exist():
                    ...
                    if tag != '':
                        ...
                        new_tag = Tag.objects.create(
                            title = tag.stript(),
                            slug = slugify(tag.stript())
                        )
                        blog.tags.add(new_tag)

            messages.success(request, "Blog adedd successfully")
            return redirect('blog_details', slug=blog.slug)
...
def update_blog(request, slug):
    blog = get_object_or_404(Blog, slug=slug)
    form = AddlogForm(instance=blog)

    if request.method == "POST":
        form = AddlogForm(request.POST, request.FILES, isinstance=blog)
        if form.is_valid():
            if request.user.pk != blog.user.pk:
                return redirect('home')
